...
class MangeRequest extends StatefulWidget {
  @override
  State<MangeRequest> createState() => _MangeRequestState();
}

class _MangeRequestState extends State<MangeRequest> {
  @override
  void initState() {
    _retriveRequest();
    super.initState();
  ...
  Future<List> _retriveRequest() async {
    try {
      ...
      var response =
          await http.get(Uri.parse('${Share.Url}getRequest'), headers: {
 ...
 Get.offNamed(AppRoute.login);
        ...
        title: Text("Admin Mange Request"),
        /* actions: [
          IconButton(
...
Text("Student Name: "),
                                ...
                                Row(
                                  ...
                                  children: [
                                    Text("Peper Name: "),
                                    Text(snapshot.data?[index]['peper']['name']),
                                  ],
                                ),
                                  Divider(),
       ...
       Map<String, dynamic> data = {
                                  ...
                                  'peper': snapshot.data?[index]['peper'],
                                  'counter': snapshot.data?[index]['counter'],
                                  'state': !snapshot.data?[index]['state'],
                                };

                                final response = await http.put(
                                    Uri.parse(
                                        '${Share.Url}updateState/${snapshot.data?[index]['id']}/'),
                                    headers: {
                                      'Content-Type': 'application/json',
                                      'Access-Control-Allow-Origin': '*',
